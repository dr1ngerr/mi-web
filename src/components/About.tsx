import { Briefcase, Building2, Workflow, Zap, type LucideIcon } from "lucide-react";
    ...
    <section id="enfoque" className="border-y border-slate-200 bg-white py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-sm font-bold uppercase tracking-wider text-indigo-800">Mi enfoque</p>
        ...
        <div className="mx-auto mt-10 grid max-w-4xl gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {aboutContent.proofs.map((proof) => (
            <div
              key={proof.label}
              className="rounded-xl border-2 border-indigo-200 bg-indigo-50 px-4 py-4 text-center"
            >
              <p className="text-xs font-bold uppercase tracking-wider text-indigo-800">
                {proof.label}
              </p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{proof.value}</p>
            </div>
          ))}
        </div>
